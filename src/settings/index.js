const baseUrl = "https://nusa-akses-backend.tiabanautoservice.com";
const setting = {
  beritaUrl: `${baseUrl}/api/post`,
  servicesUrl: `${baseUrl}/api/services`,
  settingPerusahaanUrl: `${baseUrl}/api/setting/perusahaan`,
  slidersUrl: `${baseUrl}/api/sliders`,
  pagesUrl: `${baseUrl}/api/page`,
  detailPageUrl: `${baseUrl}/api/page`,
  sendReportUrl: `${baseUrl}/api/send-report`,
  loginChatUrl: `${baseUrl}/api/chat/login`,
  getChatUrl: `${baseUrl}/api/chat`,
  sendChatUrl: `${baseUrl}/api/chat/send`,
};

export default setting;
